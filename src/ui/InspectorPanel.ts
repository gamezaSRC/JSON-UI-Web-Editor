...
export class InspectorPanel {
  ...
  render(): void {
    ...
    const header = el('div', { className: 'panel-header' },
      el('span', { className: 'panel-title' }, 'Inspector')
    );
    this.container.appendChild(header);

    if (!this.currentFile || !this.currentControl) {
      this.container.appendChild(
        el('div', { className: 'inspector-empty' }, 'Select a control to edit its properties')
      );
      return;
    }
    ...
    if (!control) {
      this.container.appendChild(
        el('div', { className: 'inspector-empty' }, 'Control not found')
      );
      return;
    }
  ...
  private updateBinding(index: number, key: keyof BindingEntry, value: unknown): void {
    ...
    if (value === undefined) {
      ...
      (control.bindings[index] as Record<string, unknown>)[key] = value;
    }
    this.projectManager.updateControl(this.currentFile, this.currentControl, control);
  }
  ...
  private editRawJSON(): void {
    ...
    const saveBtn = el('button', {
      ...
      onclick: () => {
        try {
          ...
          import('./DomUtils').then(m => m.closeModal());
          this.render();
    ...
    import('./DomUtils').then(m => m.showModal(`Edit: ${this.currentControl}`, content));
  }
}
